...
def Start():
    # show a message box if the urlib cant send any request
    try:
        ...
        messagebox.showerror('Error', f'Primero ingresa la ip de los dispositivos o haz una prueba para comenzar')
...
def show_inputs():
    ...
    try:
        for host in before_hosts:
            conn = pyphox.connect(host)
            #execute some command to check if the host is valid
            conn.execute('/config')
        frame2 = tk.Frame(Phyphox_connection)
        frame2.pack(side='bottom', pady=10)
        frame2.place(relx=0.5, rely=0.6, anchor='center')
        label.config(text= 'Haz abierto la configuración de exportación, recuerda mantener la medidición detenida antes de exportar')
        select = ttk.Combobox(frame2, values=['csv', 'excel'], state='readonly', width=20, height=3,  style='TCombobox', font=('Cascadia Code', 12), justify='center') 
        select.grid(row=0, column=0, padx=10, pady=10)
        style = ttk.Style()
        style.theme_use('default')
        style.configure('TCombobox', fieldbackground='white', background='white', foreground='black', bordercolor='black', arrowcolor='black')
        ...
        def Export():
            ...
            if select.get() == 'csv':
               if messagebox.askquestion('Exportar', 'Aun se esta trabajando en esta funcion, al presionar aceptar enviaras una petición al desarrollador para desarrollar dicha función ') =='yes':
                   ...
                   webbrowser.open('mailto:ferneyandres10@hotmail.com?subject=Exportar a csv&body=Hola, me gustaria que implementaras la funcion de exportar a csv')
                ...
                pass
                 ...
                 try:
                     ...
                     label.after(100, lambda : label.config(text = 'Los archivos se han exportado en el directorio seleccionado'))
    
                     
                 except:
                     messagebox.showerror('Error', 'Hubo un error al exportar los archivos, verifica que todo este bien')
                     print(before_hosts)
        ...
        export_button.config(bg='blue', fg='white', font=('Cascadia Code', 12, 'bold'), width=10)
        close_button = tk.Button(frame2, text='Close', command=DeleteExportInterface)
        close_button.grid(row=0, column=3, padx=10, pady=10)
        close_button.config(bg='red', fg='white', font=('Cascadia Code', 12, 'bold'), width=10)
    except:
            messagebox.showerror('Error', f'Primero ingresa la ip de los dispositivos o haz una prueba para comenzar')
            print(before_hosts)
...
def show_hosts():
    ...
    if number_of_hosts.get() !='':
        ...
        entries = []
        reset_button = tk.Button(hosts_frame, text='Cancelar', command= delete_hosts)
        reset_button.grid(row=0, column=12, padx=10, pady=10)
        reset_button.config(bg='red', fg='white', font=('Cascadia Code', 8, 'bold'), width=10)
        finish_hosts_button = tk.Button(hosts_frame, text='Finalizar', command= finish_hosts)
        finish_hosts_button.grid(row=0, column=11, padx=10, pady=10)
        finish_hosts_button.config(bg='blue', fg='white', font=('Cascadia Code', 8, 'bold'), width=10)
        ...
        messagebox.showerror('Error', 'Elige el numero de ip que vas a ingresar')
...
def finish_hosts():
    ...
    # cheking if the hosts are valid
    for host in before_hosts:
        try:
            conn = pyphox.connect(host)
            #execute some command to check if the host is valid
            ...
            messagebox.showerror('Error', f'No se pudo conectar con el servidor, verifica la dirección {host}')
    


def delete_hosts():
    #delete only the the tk.Entry widgets
    before_hosts = ['']
    ...
    label.after(1000, lambda : label.config(text = 'Selecciona el numero de IP a ingresar o haz una prueba para comenzar'))
    for widget in hosts_frame.winfo_children():
        if widget.winfo_class() == 'Entry':
            widget.destroy()
    #delete the button finalizar y reiniciar
    for widget in hosts_frame.winfo_children():
        if widget.winfo_class() == 'Button':
            widget.destroy()
def one_host():
    global one_host_entry, accept_button, cancel_button
    #delete aceptar y spinbox
    one_host_button.grid_forget()
    accept_host_button.grid_forget()
    number_of_hosts.grid_forget()
    #create button for accept the host
    label.after(100, lambda : label.config(text= 'Has entrado al modo de prueba con un solo host'))
    label.after(2000, lambda : label.config(text = 'Ingresa la el ip del host que tienes en la app'))
    accept_button = tk.Button(hosts_frame, text='Finalizar', command = finish_one_host)
    accept_button.grid(row=0, column=3, padx=10, pady=10)
    accept_button.config(bg='blue', fg='white', font=('Cascadia Code', 8, 'bold'), width=10)
    cancel_button = tk.Button(hosts_frame, text='Cancelar', command = delete_hosts)
    cancel_button.grid(row=0, column=4, padx=10, pady=10) 
    cancel_button.config(bg='red', fg='white', font=('Cascadia Code', 8, 'bold'), width=10)
    one_host_entry = tk.Entry(hosts_frame, width=15, font=('Cascadia Code', 7))
    one_host_entry.grid(row=0, column=2, padx=10, pady=10)
    one_host_entry.config(bg='white', fg='black', font=('Cascadia Code', 8, 'bold'), width=20)
...
def finish_one_host():
    ...
    try:
        ...
        #delete entry , finalizar y cancelar button 
        ...
        messagebox.showerror('Error', f'No se pudo conectar con el servidor, verifica la dirección {one_host_entry.get()}')
    #
...
#center the title and put in the top of the window
...
description.config(bg='black', fg='white')
label_number_hosts = tk.Label(Header, text='Selecciona el numero de hosts a evaluar, si deseas probar la conexion con un solo dispositivo presiona el boton Probar', font=('Cascadia Code', 8))
...
number_of_hosts = ttk.Spinbox(hosts_frame, from_=2, to=10, width=5, font=('Cascadia Code', 8), justify='center')
number_of_hosts.grid(row=0, column=0, padx=20, pady=10)
#make read only the spinbox
...
accept_host_button = tk.Button(host_frame_Add, text='Aceptar', command= show_hosts)
accept_host_button.grid(row=0, column=1, padx=20, pady=10)
accept_host_button.config(bg='blue', fg='white', font=('Cascadia Code', 8, 'bold'), width=10)
one_host_button = tk.Button(host_frame_Add, text='Probar', command= one_host)
one_host_button.grid(row=0, column=2, padx=20, pady=10)
one_host_button.config(bg='blue', fg='white', font=('Cascadia Code', 8, 'bold'), width=10)
credit_button = tk.Button(host_frame_Add, text='By: FeRsOmBrA', command= credits)
credit_button.place(x=1300, y=10)
#put the button more to the right


credit_button.config(bg='black', fg='white', font=('Cascadia Code', 8, 'bold'), width=25)
#increase box depth to 10
credit_button.config(relief='raised', borderwidth=10)
#bring button to front
credit_button.config(highlightthickness=0, highlightbackground='black', highlightcolor='black')
#create a help button
help_button = tk.Button(host_frame_Add, text='Ayuda', command= phyphox_help)
help_button.place(x=1100, y=10)
help_button.config(bg='black', fg='white', font=('Cascadia Code', 8, 'bold'), width=25)
...
#add a frame to put the buttons
...
label.after(500, lambda : label.config(text = 'Selecciona el numero de IP a ingresar o haz una prueba para comenzar'))
label.grid(row = 0 , column= 0, padx = 10, pady= 10 )
label.config( anchor ='n', font = ('Cascadia Code' , 15), pady=20)
#create a frame for show the export interface

#Create a frame that contains the buttons
...
#Create a button to stop the experiment
...
Anomaly_Detector_button = tk.Button(Phyphox_connection, text='--> Phyphox Anomaly Detector <--', command=OpenFilter)
Anomaly_Detector_button.pack(side= 'bottom', pady=10, padx=20, anchor='se')
#Create a Exit button
exit_button = tk.Button(Phyphox_connection, text='Exit', command=Phyphox_connection.destroy)
...
clear_button.config(bg='#FF0000', fg='white', font=('Cascadia Code', 20, 'bold'))
exit_button.config(bg='black', fg='white', font=('Cascadia Code', 10, 'bold'))

Anomaly_Detector_button.config(bg='red', fg='white', font=('Cascadia Code', 10, 'bold'))
